# Bonjour, je suis Géraldine Chrétien 👋

## Business Intelligence Analyst

Actuellement en formation Business Intelligence Analyst chez OpenClassrooms.

Après plusieurs années d'expérience en gestion de la paie et dans un environnement industriel international, je développe aujourd'hui mes compétences dans les domaines de la Data, de la Business Intelligence et de l'aide à la décision.

## Compétences

- SQL
- Python
- Pandas
- Power BI
- DAX
- Power Query
- Excel
- Analyse statistique
- Data Visualisation

## Projets réalisés

### Power BI
- Tableau de bord de pilotage de projets internationaux

### Python
- Analyse commerciale d'un e-commerce
- Analyse de l'évolution du marché immobilier parisien

### Data Governance
- Anonymisation RGPD de données clients

### SQL
- Analyse de performances footballistiques

## Objectif professionnel

Mettre les données au service de la prise de décision grâce à des analyses fiables, des indicateurs pertinents et des tableaux de bord adaptés aux besoins métier.


## Projets phares

### Sanitoral – Tableau de bord Power BI
Pilotage de projets internationaux, KPI, DAX, RLS et Data Storytelling.

=> Voir le projet : https://github.com/gchretien-data/sanitoral-project-dashboard

### Bottleneck – Analyse commerciale Python
Consolidation de données, analyse de rentabilité et recommandations métier.

=> Voir le projet : https://github.com/gchretien-data/bottleneck-sales-analysis


## Contact

- LinkedIn : www.linkedin.com/in/géraldine-chretien-a28b7714a

## Recherche d'opportunités

Disponible pour un poste de Business Intelligence Analyst à partir d'octobre 2026.
